...
export default (function () {
  // scheduleJob('0 45 19 * * MON-FRI', async function () {
  //   for (let i = 0; i <= 540; i++) {
  //     const date: string = moment('2023-05-10 08:00:00').tz('Asia/Seoul').format('YYYY-MM-DD HH:mm:ss');
  //     const before: string = moment(date).add(i, 'minute').toISOString(true);
  //     const now: string = moment(before)
  //       .add(i + 1, 'minute')
  //       .toISOString(true);
  ...
  //     if (craw_data.length > 0) await cRawOne.insertMany(craw_data);
  //   }
  // });
